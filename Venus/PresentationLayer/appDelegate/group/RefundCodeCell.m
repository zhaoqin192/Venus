...
- (void)awakeFromNib {
    ...
    subscribeNext:^(id x) {
        ...
        if (self.isSelected) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"CodeSubstraction" object:nil userInfo:@{@"code": self.codeLabel.text}];
        }
        else {
            self.isSelected = YES;
            [self.selectButton setImage:[UIImage imageNamed:@"select_rectange"] forState:UIControlStateNormal];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"CodeAdd" object:nil userInfo:@{@"code": self.codeLabel.text}];
